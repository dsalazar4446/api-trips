...
describe('TripsController', () => {
  ...
  it('should be created trips', () => {
    ...
    jest.spyOn(controller, 'create').mockImplementation(async () => tripMock);

    expect(controller.create(dto)).resolves.toEqual(tripMock).catch(err => {
      console.log(err);
    });
  })

  it('should be find trips', () => {
    const filters = {
      start_gte: 35141,
      start_lte: 65760,
      distance_gte: 58881,
    ...
    jest.spyOn(controller, 'find').mockImplementation(async () => tripMock);
    expect(controller.find(filters)).resolves.toEqual(tripMock).catch(err => {
      console.log(err);
    });
  })
});
